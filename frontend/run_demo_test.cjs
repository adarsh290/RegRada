...
async function resetDB() {
  ...
  console.log('🗑️ Clearing ChromaDB vector store via API...');
  try {
    const response = await fetch('http://localhost:8000/clear', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Internal-Token': 'internal-api-secret-key-regradar-backend-ai-token-2026'
      }
    });
    const resData = await response.json();
    console.log('✅ ChromaDB cleared via API:', resData.message || resData);
  } catch (err) {
    console.log('⚠️ Failed to clear ChromaDB via API:', err.message);
  }
}
...
async function run() {
  ...
  try {
    ...
    async function login(username, password) {
      ...
      // Wait for layout to load (Sign Out button is only present on dashboard)
    ...
    // Find encryption MAP
    const encryptRow = page.locator('tr').filter({ hasText: 'encryption' }).filter({ hasText: 'IT' }).first();
    ...
    // Find LSP Audit MAP
    const lspRow = page.locator('tr').filter({ hasText: 'LSP' }).filter({ hasText: 'Legal' }).first();
    ...
    await page.waitForSelector('text=Amendment Delta Report');
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, '12_phase4_delta_report_modal.png') });
    
    // Close modal
    await page.click('button[aria-label="Close delta report"]');
    ...
    // Resolve Conflict
    console.log('Resolving a conflict...');
    let conflictCard = page.locator('div.bg-gray-900').filter({ hasText: 'penal charges' }).first();
    if (await conflictCard.count() === 0) {
      console.log('No penal charges conflict found, falling back to encryption conflict...');
      conflictCard = page.locator('div.bg-gray-900').filter({ hasText: 'encryption' }).first();
    }
    if (await conflictCard.count() === 0) {
      console.log('No encryption conflict found, falling back to first available conflict...');
      conflictCard = page.locator('div.bg-gray-900').filter({ hasText: 'Resolve Conflict' }).first();
    }
    ...
    await page.waitForSelector('text=Search Results', { timeout: 90000 });
    await page.waitForTimeout(3000);
    ...
    await page.waitForSelector('text=Obligation Graph');
